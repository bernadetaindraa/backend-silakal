...
<div class="p-6 space-y-6 max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden p-6">
        ...
        <form action="{{ route('admin.produk-hukum.update', $produkHukum->produk_hukum_id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" 
                           name="nama_dokumen" 
                           id="nama_dokumen"
                           value="{{ old('nama_dokumen', $produkHukum->nama_dokumen) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="kategori_dokumen" 
                        ...
                        <option value="Perencanaan Penganggaran" {{ old('kategori_dokumen', $produkHukum->kategori_dokumen) == 'Perencanaan Penganggaran' ? 'selected' : '' }}>Perencanaan Penganggaran</option>
                        <option value="Peraturan Kalurahan" {{ old('kategori_dokumen', $produkHukum->kategori_dokumen) == 'Peraturan Kalurahan' ? 'selected' : '' }}>Peraturan Kalurahan</option>
                        <option value="Laporan" {{ old('kategori_dokumen', $produkHukum->kategori_dokumen) == 'Laporan' ? 'selected' : '' }}>Laporan</option>
                        <option value="Peraturan Lurah" {{ old('kategori_dokumen', $produkHukum->kategori_dokumen) == 'Peraturan Lurah' ? 'selected' : '' }}>Peraturan Lurah</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" 
                           name="nomor_dokumen" 
                           id="nomor_dokumen"
                           value="{{ old('nomor_dokumen', $produkHukum->nomor_dokumen) }}"
                ...
                <div>
                    ...
                    <input type="date" 
                           ...
                           value="{{ old('tanggal_ditetapkan', \Carbon\Carbon::parse($produkHukum->tanggal_ditetapkan)->format('Y-m-d')) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="tipe_dokumen" 
                        ...
                        <option value="PDF" {{ old('tipe_dokumen', $produkHukum->tipe_dokumen) == 'PDF' ? 'selected' : '' }}>PDF</option>
                        <option value="Docx" {{ old('tipe_dokumen', $produkHukum->tipe_dokumen) == 'Docx' ? 'selected' : '' }}>Word (Docx)</option>
                        <option value="Excel" {{ old('tipe_dokumen', $produkHukum->tipe_dokumen) == 'Excel' ? 'selected' : '' }}>Excel</option>
                        
                        {{-- PERBAIKAN: Ubah value="Link" menjadi value yang sebenarnya --}}
                        <option value="JPG" {{ old('tipe_dokumen', $produkHukum->tipe_dokumen) == 'JPG' ? 'selected' : '' }}>JPG</option>
                        <option value="PNG" {{ old('tipe_dokumen', $produkHukum->tipe_dokumen) == 'PNG' ? 'selected' : '' }}>PNG</option>
                        <option value="JPEG" {{ old('tipe_dokumen', $produkHukum->tipe_dokumen) == 'JPEG' ? 'selected' : '' }}>JPEG</option>
                        
                        <option value="Link" {{ old('tipe_dokumen', $produkHukum->tipe_dokumen) == 'Link' ? 'selected' : '' }}>Tautan / Link Eksternal</option>
                    </select>
                    @error('tipe_dokumen')
                        <p class="text-xs text-red-500 mt-1 font-medium">{{ $message }}</p>
                    @enderror
                ...
                <div id="container_url" class="md:col-span-2 hidden">
                    ...
                    <input type="url" 
                           name="url_dokumen" 
                           id="url_dokumen"
                           value="{{ old('url_dokumen', $produkHukum->tipe_dokumen == 'Link' ? $produkHukum->url_dokumen : '') }}"
                ...
                <div id="container_file" class="md:col-span-2 hidden">
                    ...
                    @if($produkHukum->tipe_dokumen != 'Link' && $produkHukum->url_dokumen)
                        <div class="mb-3 px-4 py-3 bg-blue-50 border border-blue-100 rounded-xl text-sm flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-blue-800 font-medium">File saat ini terlampir</span>
                            </div>
                            <a href="{{ $produkHukum->url_dokumen }}" target="_blank" class="text-blue-600 hover:text-blue-800 font-semibold underline">Lihat File</a>
                        </div>
                    @endif
                    ...
                    <p class="text-xs text-gray-400 mt-1">Kosongkan jika tidak ingin mengganti file. Format file harus sesuai dengan tipe dokumen yang dipilih (Maks. 10MB)</p>
                    @error('file_dokumen')
                        <p class="text-xs text-red-500 mt-1 font-medium">{{ $message }}</p>
                    @enderror
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function handleTipeDokumenChange() {
            ...
            // PERBAIKAN: Tambahkan pengecekan format gambar di sini
        ...
        // Memanggil fungsi saat halaman pertama kali di-load
