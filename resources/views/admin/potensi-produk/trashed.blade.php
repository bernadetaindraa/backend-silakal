    ...
    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-[#1D2059]">
                Sampah Potensi Produk
            </h1>

            <p class="text-sm text-gray-500 mt-1">
                Daftar potensi produk yang telah dihapus sementara
            </p>
        </div>

        <a href="{{ url('admin/potensi-produk') }}"
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    {{-- RESTORE --}}
                                    <form
                                        action="{{ url('admin/potensi-produk/restore/' . $item->potensi_produk_id) }}"
                                        method="POST"
                                    >
                                        @csrf
                                    ...
                                    {{-- DELETE PERMANENT --}}
                                    <form
                                        action="{{ url('admin/potensi-produk/force-delete/' . $item->potensi_produk_id) }}"
                                        method="POST"
                                        onsubmit="return confirm('Hapus permanen data potensi produk ini?')"
                                    >
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        ...
                        <tr>
                            ...
                            <td colspan="4" class="px-6 py-10 text-center text-gray-400">
                                ...
                                Belum ada data potensi produk di sampah
